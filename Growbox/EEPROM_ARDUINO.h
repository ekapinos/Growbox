...
public:
  ...
  template<class T>
    void readBlock(const word address, T value[], word items) {
  ...
  template<class T>
    T readBlock(const word address) {
  ...
  template<class T>
    word writeBlock(word address, const T value[], word items) {
      if ((address + items * sizeof(T) - 1) >= getCapacity()) {
        return 0;
      }
      word rez;
  ...
  template<class T>
    word writeBlock(const word address, const T& value) {
  ...
  template<class T>
    word updateBlock(const word address, const T value[], word items) {
  ...
  template<class T>
    int updateBlock(word address, const T& value) {
      word writeCount = 0;
      if ((address + sizeof(value) - 1) >= getCapacity()) {
        return 0;
      }
